...
weather_stations <- read.csv("weatherstations")
...
tidal_stations <- read.csv("tidalstations")
...
# Use moonillumination function from suncalc package
manta_df$moonfraction <- getMoonIllumination(date = manta_df$datetime, keep = c("fraction"))
manta_df$moonphase <- getMoonIllumination(date = manta_df$datetime, keep = c("phase"))

## SUN =========================================================================
manta_df$sun <- getSunlightPosition(data = manta_df, keep = c("altitude"))

# Convert altitude from radians to degrees
manta_df$sun$altitude <- manta_df$sun$altitude * (180 / pi)
