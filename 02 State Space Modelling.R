...
ssm_all <- 
  fit_ssm(all_data, # Filtered to just look at one tag
...
# Grab the data from the model to produce a data frame
...
ssm_manta_sf <- st_as_sf(ssm_all_df, coords = c("lon", "lat"), crs = st_crs(nzcoast))
